...
.fun_Descendants_node_labels <- function(tree, node, type = "tips"){
  ...
  node_labels_in_edge <- tree$node.label[tree$edge[,1]-ape::Ntip(tree)]
  ...
  nodes_to_keep_number <- node %>%
    as.data.frame() %>%
    dplyr::rename(parent.name = ".") %>%
    dplyr::inner_join(edge_table, by = "parent.name") %>%
    dplyr::select("parent")[,1] %>%
    unique()
  ...
  descendants_tips_labels <- descendants_node_numbers %>%
    as.data.frame() %>%
    dplyr::rename(child = ".") %>%
    dplyr::inner_join(edge_table, by = "child") %>%
    dplyr::select("child.name")[,1] %>%
    unique()
...
keep_as_tip<- function(tree = NULL,
  ...
  nodes_to_keep <-  nodes_df %>%
    dplyr::rename(tips_and_nodes_to_keep = ".") %>%
    dplyr::inner_join(as.data.frame(tips_and_nodes_to_keep), by = "tips_and_nodes_to_keep")[,1]
  ...
  if (n_descdendants_in_keep_list > 0) {
    ...
    # going through the internal nodes one by one and pruning away descendant tips. It has to be tips that are pruned each time because ape::drop.tip() only takes tips. So the while-loop works "upwards" in the tree until it hits the node in question and then stops.
    ...
    tree <- ape::drop.tip(tree, tip = tips_to_drop, collapse.singles = F)
    ...
    joined <- tree$tip.label %>%
      as.data.frame() %>%
      dplyr::rename(tips_and_nodes_to_keep = ".") %>%
      dplyr::inner_join(as.data.frame(tips_and_nodes_to_keep), by = "tips_and_nodes_to_keep")
